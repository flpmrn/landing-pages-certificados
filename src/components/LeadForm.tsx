...
export default function LeadForm({ product, formCopy }: { product: string, formCopy: any }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Use the custom message from copy
        const message = formCopy.whatsappMessage || `Olá! Tenho interesse no certificado digital ${product}. Meu nome é ${formData.name}.`;
        const wppNumber = "5516988443346";

        // Redirect to WhatsApp API
        window.location.href = `https://wa.me/${wppNumber}?text=${encodeURIComponent(message)}`;
    };
